...
public class ECommand implements CommandExecutor {
	 ...
	 * 
	 * @param executor
	 *            The CommandExecutor if there's no subcommand used
	 * @param helpMessage
	 *            The help message, for if there's a problem with the sub
	 *            command structure
	 * @param useArguments
	 *            Whether the command should use arguments or sub commands
	 */
	 ...
	 */
	public ECommand(CommandExecutor executor, String helpMessage,
			HashMap<String, ECommand> subCommands) {
		this(executor, helpMessage, true);

		for (String subCommand : subCommands.keySet()) {
			this.subCommands.put(subCommand.toLowerCase(),
		...
		if (useArguments) {
			if (args.length > 0) {
				if (subCommands.containsKey(args[0])) {
					...
					// Create a List< String > with all the arguments for the
					// sub command
					...
					// Run the command with the subArguments list converted back
					// to an String[] array
					return subCommands.get(args[0]).onCommand(sender, command,
							label,
							(String[]) subArguments.toArray(new String[0]));
				} else {
	 ...
	 * 
	 * @param name
	 *            The name of the sub command. for example /test [name]
	 * @param subcommand
	 *            The ECommand instance that will be run when the subcommand is
	 *            run
	 * @throws IllegalStateException
	 *             When useArguments (in the constructor without the subCommands
	 *             HashMap) is true
	 */
	public ECommand addSubCommand(String name, ECommand subcommand)
			throws IllegalStateException {
		if (!useArguments) {
			this.subCommands.put(name.toLowerCase(), subcommand);
		} else {
			throw new IllegalStateException(
					"Can't use subcommands when arguments are used");
		}
		return this;
	}
